...
function pexec(re, subject, offset, options, match_data)
    rc = ccall((:pcre2_match_8, Base.PCRE.PCRE_LIB), Cint,
               ...
               re, subject, sizeof(subject), offset, options, match_data, Base.PCRE.get_local_match_context())
    # rc == -1 means no match, -2 means partial match.
    rc < -2 && error("PCRE.exec error: $(err_message(rc))")
    rc
end

# Mirrors Base.PCRE.exec_r_data: helper to call pexec w/ thread-safe match data
